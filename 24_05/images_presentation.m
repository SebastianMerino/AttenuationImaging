...
resultsDir = 'C:\Users\sebas\Pictures\ISBI2024\presentation';
...
figure('Units','centimeters', 'Position',[5 5 17 5])
...
t2 = nexttile; 
imagesc(x_ACS,z_ACS,abs(CRTVL1), [0 20])
colormap(t2,parula)
axis image
title('|\DeltaBSC|')
c = colorbar(t2, cbPlacement);
...
%%
bscExample = -25:0.1:25;
wExample =  (1-reject)*(1./((bscExample/ratioCutOff).^(2*order) + 1))+reject;
figure('Units','centimeters', 'Position',[5 5 7 3])
plot(bscExample,wExample, 'LineWidth',1)
ylim([0,1])
xlim([bscExample(1) bscExample(end)])
xlabel('\Delta BSC')
ylabel('w')
xline(ratioCutOff,'k--')
xline(-ratioCutOff,'k--')
