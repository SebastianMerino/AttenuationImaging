% Script to test resolution of each approach.

clear,clc

targetDir = ['C:\Users\sebas\Documents\MATLAB\DataProCiencia\' ...
    'Attenuation\Simulation\24_06_06'];
...
% New simu
referenceAtt    = 0.6;
groundTruthBack = [0.5,0.5,0.5,0.5,0.5,0.5];
groundTruthInc = [1,1,1,1,1,1];
...
if iAcq ==1
    ...
    for iRef = 1:Nref %Nref
        ref = load(fullfile(refDir,refFiles(iRef).name),"rf","medium");
        acs_mean = ref.medium.alpha_coeff(1,1);
        att_ref = acs_mean*(f.^ref.medium.alpha_power)/NptodB;
...
% figure, imagesc(x_ACS,z_ACS,attIdealACS{iAcq}, attRange)
% axis image
% colormap turbo
...
t2 = nexttile;
imagesc(rx(1,1:end)*100,rz(1:end,1)*100,medium.alpha_coeff,attRange)
...
%%
axialTV{iAcq} = mean(BRTV(:,41:48),2);
axialSWTV{iAcq} = mean(BRSWTV(:,41:48),2);
axialTVL1{iAcq} = mean(BRTVL1(:,41:48),2);
axialWFR{iAcq} = mean(BRWFR(:,41:48),2);

lateralTV{iAcq} = mean(BRTV(23:25,:),1);
lateralSWTV{iAcq} = mean(BRSWTV(23:25,:),1);
lateralTVL1{iAcq} = mean(BRTVL1(23:25,:),1);
lateralWFR{iAcq} = mean(BRWFR(23:25,:),1);
...
nexttile,
plot(z_ACS, axialTV{iAcq}, ':', 'LineWidth',1.5, 'Color',lineColors(1,:) ),
hold on
plot(z_ACS, axialSWTV{iAcq}, '-', 'LineWidth',1, 'Color',lineColors(1,:)),
plot(z_ACS, axialTVL1{iAcq}, ':', 'LineWidth',1.5, 'Color',lineColors(2,:)),
plot(z_ACS, axialWFR{iAcq}, '-', 'LineWidth',1, 'Color',lineColors(2,:)),
plot(z_ACS,mean(attIdealACS{iAcq}(:,41:49),2), 'k--')
hold off
grid on
ylim([0.4 1.1])
...
xlabel('Axial [cm]')
if iAcq==1, legend({'TV','SWTV','TVL1','WFR'}, 'Location','northwest'); end

nexttile,
plot(x_ACS, lateralTV{iAcq}, ':', 'LineWidth',1.5, 'Color',lineColors(1,:) ),
hold on
plot(x_ACS, lateralSWTV{iAcq}, '-', 'LineWidth',1, 'Color',lineColors(1,:)),
plot(x_ACS, lateralTVL1{iAcq}, ':', 'LineWidth',1.5, 'Color',lineColors(2,:)),
plot(x_ACS, lateralWFR{iAcq}, '-', 'LineWidth',1, 'Color',lineColors(2,:)),
plot(x_ACS,mean(attIdealACS{iAcq}(24:26,:),1), 'k--')
hold off
grid on
ylim([0.4 1.1])
xlim([x_ACS(1) x_ACS(end)])
...
xlabel('Lateral [cm]')
% legend({'TV','SWTV','TVL1','WFR'}, 'Location','northeast') 
