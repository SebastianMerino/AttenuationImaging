...
baseDir = ['C:\Users\sebas\Documents\MATLAB\DataProCiencia\' ...
    'Attenuation\Liver'];
targetDir = fullfile(baseDir,'new');
refsDir = fullfile(baseDir,'newRef');
resultsDir = fullfile(targetDir,'results','test');
...
% rect = [0.1, 3, 3.6, 5]; % Large
rect = [0.3, 3, 3.2, 3]; % Small

% Bandwidth
fixedBW = false;
ratio = db2mag(-20);
% freq_L = 1.1e6; freq_H = 5.8e6;
freq_L = 1.2e6; freq_H = 6.1e6;
...
nexttile, imagesc(xFull(1:2:end),zFull,BmodeFull(:,1:2:end), dynRange)
    colormap gray; 
    hb2=colorbar; ylabel(hb2,'dB')
    xlabel('\bfLateral distance (cm)'); ylabel('\bfAxial distance (cm)');
    title('Liver')
axis image
nexttile, imagesc(xFull(2:2:end),zFull,BmodeFull(:,2:2:end), dynRange)
    colormap gray; 
    hb2=colorbar; ylabel(hb2,'dB')
    xlabel('\bfLateral distance (cm)'); ylabel('\bfAxial distance (cm)');
    title('Liver')
...
clear Sp_ref Sd_ref
%% Image of compensation
% figure,
% for iFreq= 1:p
%     imagesc(x_ACS,z_ACS,compensation(:,:,iFreq), [-2 2])
%     axis image
%     colorbar
%     pause(0.1)
% end
% 
figure,
imagesc(x_ACS,z_ACS,mean(compensation,3), [-2 2])
axis image
colorbar
pause(0.1)
...
%%
sldNotComp = log(Sp) - log(Sd);

figure,
for iFreq= 1:p
    imagesc(x_ACS,z_ACS,sldNotComp(:,:,iFreq), [-2 2])
    axis image
    colorbar
    pause(0.1)
end
