...
baseDir = ['C:\Users\sebas\Documents\MATLAB\DataProCiencia\' ...
    'Attenuation\Simulation\24_02_04'];
...
groundTruthBack = [1.5,1.5];
groundTruthInc = [0.8,0.8];
...
% %% Ideal b
% bIdeal = attIdeal.*(reshape(f,[1,1,p]).^1.05)/NptodB;
% % figure,imagesc(squeeze(bIdeal(20,:,:)), [0 2])
% % colorbar
% %figure,imagesc(squeeze(b(20,:,:)), [0 2])
% % colorbar
% 
% bError = b - bIdeal;
% %figure,imagesc(squeeze(bError(20,:,:)), [-1,1])
% bMse = squeeze(std(bError,[],3).^2);
% figure,imagesc(x_ACS,z_ACS,bMse)
% colorbar
% axis image
%% Spectrum
% Heterogeneous
...
% %% error in fit
% fid = A1*Bn + A2*Cn - b(:);
% fid = reshape(fid,[m n p]);
% fidMap = squeeze(std(fid,[],3).^2);
% imagesc(x_ACS,z_ACS,fidMap)
% colorbar
% axis image
